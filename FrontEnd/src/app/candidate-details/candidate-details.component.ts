...
export class CandidateDetailsComponent implements OnInit {
  ...
  GetCandidateData() {
    ...
    this.mappingService.GetAllCandidateMappingData().subscribe(
      (res) => {
        this.mappinglst = res;
    ...
    this.service.GetAllCandidatesData().subscribe(
      (data) => {
        ...
        this.totalPages = Math.ceil(
          this.CandidateList.length / this.pageSizeSelected
        );
  ...
  onEdit() {
    ...
    this.service.UpdateCandidateData(this.Id, obj).subscribe(
      (res) => {
  ...
  editDetails(candidate: any) {
    ...
    this.router.navigate(["/candidateList"], {
      queryParams: { editMode: this.editmode, myArray: this.candidate },
    });
